    ...
    if has_name and has_service and has_date:
        async with AsyncSession() as db:
            try:
                ...
                elif session.get("appointment_date"):
                    try:
                        found_slot, parsed_slot_date, parsed_slot_time = await schedule_service.resolve_slot_from_text(session.get("appointment_date"))
                        if parsed_slot_date:
                            slot_date_obj = parsed_slot_date
                            session["slot_date"] = parsed_slot_date.isoformat()
                        if parsed_slot_time:
                            raw_slot_time = parsed_slot_time
                            session["slot_time"] = parsed_slot_time
                        if found_slot:
                            slot_id = found_slot.id
                    except Exception as e:
                        logger.error(f"Erro ao inferir slot a partir do texto do agendamento: {e}")
